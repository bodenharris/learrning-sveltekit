...
<div class="min-h-screen {bgColor} p-8 {textColor}">
    ...
    <p class=" mt-4 border {borderColor} rounded-lg w-1/2 h-auto p-1" >
        ...
        <button 
        class="fixed w-16 h-16 right-4 flex items-center justify-center rounded-full {bgColor} {hoverColor} focus:outline-none focus:ring-2 focus:ring-blue-500"
        aria-label="Download Resume">
            <img src=downloadIcon.png alt="Download Icon" class="w-[40px] h-[40px]" style="{nightMode ? 'filter: invert(1)' : ''}">
        </button>
    </p>
